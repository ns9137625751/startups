...
<div class="mb-6 flex items-center justify-between">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-extrabold" style="color:#0d1b2a;">Mentor Profiles</h1>
    ...
    <div class="flex items-center gap-2 flex-wrap">
        ...
        <!-- Import trigger -->
        <button onclick="document.getElementById('mentor-import-modal').style.display='flex'"
...
<div id="mentor-import-modal"
    ...
    <div style="background:#fff;border-radius:16px;padding:28px 32px;width:100%;max-width:480px;box-shadow:0 20px 60px rgba(0,0,0,0.15);">
        <div class="flex items-center justify-between mb-5">
            <h2 class="text-base font-extrabold" style="color:#0d1b2a;">Import Mentor Profiles</h2>
            <button onclick="document.getElementById('mentor-import-modal').style.display='none'"
        ...
        <div class="mb-4 p-3 rounded-lg text-xs font-semibold" style="background:#fffbeb;color:#92400e;border:1px solid #fde68a;">
            <p class="font-extrabold mb-1">&#9888; Excel column order (row 1 = heading, data from row 2):</p>
            <p>name, email, mobile_number, organization, designation, brief, domain, expertise</p>
        </div>

        <div class="mb-4">
            <a href="{{ route('dashboard.super-admin.mentors.template') }}"
               class="text-xs font-bold flex items-center gap-1" style="color:#1F3C88;">
        ...
        <form method="POST" action="{{ route('dashboard.super-admin.mentors.import') }}" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
            ...
            <div class="flex items-center gap-2 justify-end">
                <button type="button" onclick="document.getElementById('mentor-import-modal').style.display='none'"
...
<div class="mb-5 flex items-start gap-3 flex-wrap">
    <div class="relative" style="min-width:260px;">
        ...
        <input id="search-input" type="text" placeholder="Search name, email, org, domain..."
        ...
        <table class="w-full text-sm">
            <thead style="background:#f9fafb;">
                <tr>
                    ...
                    <th class="text-left py-3 px-4 text-xs font-extrabold" style="color:#0d1b2a;">Name</th>
                    ...
                    <th class="text-left py-3 px-4 text-xs font-extrabold" style="color:#0d1b2a;">Organization</th>
                    <th class="text-left py-3 px-4 text-xs font-extrabold" style="color:#0d1b2a;">Designation</th>
                    <th class="text-left py-3 px-4 text-xs font-extrabold" style="color:#0d1b2a;">Domain</th>
                    <th class="text-left py-3 px-4 text-xs font-extrabold" style="color:#0d1b2a;">Expertise</th>
            ...
            <tbody id="mentors-tbody">
                <tr><td colspan="10" class="py-16 text-center text-xs font-semibold" style="color:#6b7280;">Loading...</td></tr>
            </tbody>
        </table>
    </div>
...
<script>
const SEARCH_URL   = '{{ route("dashboard.super-admin.mentors.search") }}';
const LS_KEY       = 'sa_mentors_perPage';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if(session('error'))
    document.getElementById('mentor-import-modal').style.display = 'flex';
    @endif
    fetchData();
});
...
function fetchData() {
    ...
    const params = new URLSearchParams({ q, filter: currentFilter, page: currentPage, perPage: currentPerPage });
    document.getElementById('mentors-tbody').innerHTML =
        '<tr><td colspan="10" class="py-16 text-center text-xs font-semibold" style="color:#6b7280;">Loading...</td></tr>';
    fetch(SEARCH_URL + '?' + params.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => r.json()).then(renderTable)
        .catch(() => {
            document.getElementById('mentors-tbody').innerHTML =
                '<tr><td colspan="10" class="py-16 text-center text-xs font-semibold" style="color:#ef4444;">Failed to load data.</td></tr>';
        });
}

function renderTable(data) {
    document.getElementById('total-count').textContent = data.total + ' total mentors';
    const tbody = document.getElementById('mentors-tbody');
    if (!data.rows.length) {
        tbody.innerHTML = `<tr><td colspan="10" class="py-16 text-center"><p class="font-extrabold text-sm" style="color:#0d1b2a;">No mentor profiles found</p><p class="text-xs font-semibold mt-1" style="color:#6b7280;">No submissions yet</p></td></tr>`;
        ...
        <tr style="border-bottom:1px solid #f3f4f6;" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background=''">
            ...
            <td class="py-3 px-4">
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-extrabold text-white flex-shrink-0"
                         style="background:linear-gradient(135deg,#9333ea,#7e22ce);">${r.initial}</div>
                    <p class="font-extrabold text-xs" style="color:#0d1b2a;">${r.name}</p>
                </div>
            </td>
            <td class="py-3 px-4 text-xs font-semibold" style="color:#4b5563;">${r.email}</td>
            <td class="py-3 px-4 text-xs font-semibold" style="color:#4b5563;">${r.mobile}</td>
            <td class="py-3 px-4 text-xs font-semibold" style="color:#0d1b2a;">${r.organization}</td>
            <td class="py-3 px-4 text-xs font-semibold" style="color:#4b5563;">${r.designation}</td>
            <td class="py-3 px-4">
                <span class="px-2 py-1 rounded-full text-xs font-bold" style="background:#fdf4ff;color:#9333ea;">${r.domain}</span>
            </td>
            <td class="py-3 px-4 text-xs font-semibold" style="color:#4b5563;">${r.expertise}</td>
            ...
            <td class="py-3 px-4">
                <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    <form method="POST" action="${r.deleteUrl}" onsubmit="return confirm('Delete this mentor?')">
...
function approveMentor(url, csrf) {
    fetch(url, {
        method: 'POST',
